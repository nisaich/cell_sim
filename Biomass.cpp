...
bool active_Biomass::reproduction(Field& current_field, int x, int y) {
    ...
    // --- tip-biased growth: подавляем деление у клеток, зажатых соседями ---
    // exposure = 1.0 у клетки на открытом кончике ветки (все 4 соседа свободны),
    // exposure -> 0 у клетки в узкой впадине/углублении контура (1 свободный сосед).
    // Это усиливает естественную нехватку питания во впадинах и не даёт колонии
    // "зарастать" изнутри, превращая фронт роста в ветвящиеся дендриты.
    {
        double exposure = static_cast<double>(free_neighbours.size()) / 4.0;
        double min_factor = simulation_config::biomass::branching_min_factor;
        double exposure_multiplier = min_factor + (1.0 - min_factor) * exposure;
        current_chance *= exposure_multiplier;
    }
