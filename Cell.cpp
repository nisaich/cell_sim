...
void nonactive_Cell::depletion_of_savings() {
    // Неактивная клетка тратит значительно меньше еды за шаг
    food_inside -= static_cast<int>(using_food_for_step * food_usage_multiplier);
    
    // На всякий случай проверяем, чтобы еда не ушла в минус меньше нуля
    if (food_inside < 0) {
        food_inside = 0;
    }
}
