...
namespace simulation_config {
	...
	namespace biomass {
		...
		inline constexpr float nonactive_resistance_multiplier = 0.0f;
	...
	namespace antibiotic {
		inline constexpr float diffusion_coeff = 0.15f;
		inline constexpr float adding_concentration = 10.0f;
		inline constexpr int adding_interval = 50;
		inline constexpr float visualization_normalizer = 10.0f;

		// Новые параметры для влияния на клетки
		inline constexpr float death_threshold = 0.1f;          // превышение над резистентностью для начала гибели
		inline constexpr float death_probability_factor = 0.05f; // множитель вероятности смерти на единицу превышения
		inline constexpr float reproduction_penalty = 0.5f;      // максимальный штраф к шансу деления (0..1)
		inline constexpr float stress_transition_chance = 0.02f; // шанс перехода в неактивное состояние за шаг
	}
