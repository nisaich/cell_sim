...
namespace simulation_config {
    ...
    // ============================================================
    // Этот сценарий моделирует популяцию P. putida с чрезвычайно эффективными
    // механизмами защиты (гиперактивные эффлюксные насосы TtgABC с низким штрафом).
    //
    // Настройки для этого режима:
    // 1. Постоянная подача огромных доз антибиотика (среднее значение 8.0 при MIC = 2.0).
    // 2. Высокая базовая устойчивость: default_resistance = 1.0 (в 70 000 раз выше нормы!).
    // 3. Мгновенная индукция насосов: k_ind = 0.50 (выходят на максимум за 1 минуту).
    // 4. Почти нулевое расслабление: k_rec = 0.001 (насосы постоянно открыты).
    // 5. Минимальный фитнес-штраф за насосы: fitness_cost_coef = 0.02 (тратят всего 2% энергии).
    // 6. Устойчивость к стрессу при делении: reproduction_penalty = 0.90 (делятся почти без штрафа).
    // ============================================================

    namespace monod {
        inline constexpr double delta_t = 60.0;

        inline constexpr double U_max = 0.613 / 3600.0;
        inline constexpr double m_act    = 0.024 / 3600.0;
        inline constexpr double m_inactiv = 0.024 / 360000.0;

        inline constexpr double K_F = 50.0;
